...
from train_mappo_crl2 import MAPPOActorCritic
from multiagent_minigrid_env import MultiAgentGoalReachingEnv


def visualize_mappo(model_path, grid_size=8, max_steps=100, delay=0.3, log_dir="./tensorboard/mappo/mappo_2/", n_episodes=1):
    """Visualize and log MAPPO agent behavior in MultiAgent MiniGrid across multiple episodes"""
    ...
    for ep in range(n_episodes):
        ...
        print(f"\n=== MAPPO Visualization | Episode {ep + 1}/{n_episodes} ===")
        ...
        while not (all(terminated.values()) or all(truncated.values())):
            ...
            ax.set_title(f'Episode {ep + 1}/{n_episodes} | Step {steps}/{max_steps}')
        ...
        writer.add_scalar('Evaluation/TotalReward', total_reward, ep)
        writer.add_scalar('Evaluation/Success', 1.0 if success else 0.0, ep)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Visualize and log MAPPO-trained agents across multiple episodes')
    parser.add_argument('--model', type=str, default='./checkpoints/mappo/mappo_2/mappo_update_240.pt',
    ...
    parser.add_argument('--episodes', type=int, default=50, help='Number of episodes to visualize')

    args = parser.parse_args()

    visualize_mappo(args.model, args.grid_size, args.max_steps, args.delay, n_episodes=args.episodes)
